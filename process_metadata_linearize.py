...
# Mapping from book code or folder to metadata details
BOOK_METADATA_MAP = {
    "First_Flight": {
        "subject": "English",
...
def determine_file_details(folder_name, filename):
    ...
    if "prelims" in fname_lower or fname_lower.endswith("ps.pdf") or fname_lower.endswith("pr.pdf"):
        ...
        # Check chapter number: e.g. jeff101.pdf -> 1, jemh114.pdf -> 14
...
def process_pdf(pdf_path, folder_name):
    ...
    temp_output = pdf_path + ".linear.tmp"
    
    with pikepdf.open(pdf_path) as pdf:
        # 1. Clear any unwanted third-party tool metadata in docinfo
        ...
        # 2. Synchronize XMP metadata without pikepdf editor stamp
        ...
        # 3. Save with linearization enabled (Fast Web View)
...
def main():
    print("=" * 60)
    print("STARTING METADATA SANITIZATION & FAST WEB VIEW OPTIMIZATION")
    print("=" * 60)
    ...
    for root, dirs, files in os.walk(BASE_DIR):
        ...
        # Book folder is either parts[1] (if in chapters/) or parts[1] (if in book folder root)
        if len(parts) >= 2:
            book_folder = parts[1]
        ...
        for f in files:
            if f.lower().endswith('.pdf'):
                ...
                total_processed += 1
                print(f"[{total_processed:03d}] Optimized & Linearized: {f} -> '{title}'")
                
    elapsed = time.time() - start_time
    print("\n" + "=" * 60)
    print("METADATA & FAST WEB VIEW OPTIMIZATION COMPLETE")
    print(f"Total PDFs Processed: {total_processed}")
    print(f"All files linearized with Fast Web View enabled.")
